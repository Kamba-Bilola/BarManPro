...
const UnpaidStock = () => {
 ...
 return (
    <div className="container-fluid m-0 p-0">
      ...
      <div className="client-table-menu-bar d-flex flex-wrap justify-content-between align-items-center">
         <div className="d-flex flex-wrap justify-content-center">
          {tables.map(table => (
            <div key={table.id} className="table-box mx-2 mb-2">
              <h5 className="text-center">Table {table.id}</h5>
              <Button variant="success" className="mb-2 w-100" onClick={() => handleAddClient(table.id)}>+</Button>
              {table.clients.map(client => (
                <div key={client.id} className="client-box mt-2">
                  <Button variant="outline-info" className="w-100 mb-1" onClick={() => setSelectedClientId(client.id)}>
                    Client {client.id} ({client.balance} FCFA)
                  </Button>
                  <Button variant="outline-warning" className="w-100" onClick={() => handlePayClientBalance(table.id, client.id)}>
                    Pay
                  </Button>
                </div>
              ))}
      ...
      <div className="stock-update-section">
        <div className="row">
          {items.map((item) => {
            ...
            return (
              <div key={item.id} className="col-6 col-sm-4 col-md-3 mb-3">
