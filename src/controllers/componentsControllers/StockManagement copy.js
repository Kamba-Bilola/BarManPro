...
const StockManagement = () => {
 ...
 let DBProduct= {
  ...
  const handleSubmitProduct = async (product) => {
    ...
    try {
      const uploadImage = async (file) => {
        ...
        return result.imagePath; // Path to save in the product object
      };
  ...
  DBProduct = { name:product.name,category: product.category,price: product.price,image: product.image,capacity: product.defaultAttributes.capacity, packaging: product.defaultAttributes.packaging,flavor: product.defaultAttributes.flavor};
 
  const lastId = await getLastIdAndSet("Products"); 
  const newId = lastId !== null && lastId !== undefined ? lastId : 1;
  console.log("---------------");
  console.log(DBProduct , newId);
