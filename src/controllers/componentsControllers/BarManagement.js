    ...
    const handleSaveBar = async () => {
        if (editIndex !== null) {
            ...
            try {
                ...
                const updatedBars = [...barList];
                updatedBars[editIndex] = newBar;
                setBarList(updatedBars);
        } catch (error) {
            console.error('Error updating bar:', error);
            setNotification({
                type: 'error',
                messages: ["Failed to update bar", error.message]
            });
            ...
            try {
                ...
                if (!mainUserUid) {
                    setNotification({ type: 'error', messages: ["Error getting the main user details"] });
                    return; // Stop execution if UID is not found
                }
                ...
                try {
                    // Check for existing permissions related to the bar and user
                    ...
                    if (recordToChange && recordToChange.length > 0) {
                        ...
                        myPermis = {
                            id: recordToChange[0].id,
                            userId: mainUserUid,
                            barId: newBar.uid,
                            grantedBy: newBar.ownerUid,
                            grantedAt: currentDate,
                            isMainBar: isMainBar // Ensure `isMainBar` is set correctly
                        };
                        ...
                        setNotification({ type: 'error', messages: ["No permission record found for this user and bar"] });
                    }
                } catch (error) {
                    console.error('Error updating permissions:', error);
                    setNotification({ type: 'error', messages: ["Failed to update permissions", error.message] });
                }
        
            } catch (error) {
                console.error('Error loading main user:', error);
        ...
        loadMainBarListExec(mainBarList, setMainBarList);
        resetBarForm();
        setShowModal(false);
    };
...
const handleEditBar = (index) => {   
    const barToUpdate = barList[index];
    setEditIndex(index);
...
const handleMainBarRadioChange = (e) => {
    ...
    return (
        <div>
            ...
            {role === "Bartender" && barList !== undefined && (<BarTender
